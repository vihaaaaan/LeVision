...
from .espn_client import parse_espn_datetime_utc, parse_event_id_from_url, to_eastern
...
def build_game_row(
    ...
    game_date_raw = competition.get("date") or summary.get("gameInfo", {}).get("gameDate")
    game_date = game_date_raw
    game_dt_utc = parse_espn_datetime_utc(game_date_raw)
    if game_dt_utc is not None:
        game_date = to_eastern(game_dt_utc).isoformat()
    status = (
...
def get_play_by_play_from_summary(summary: dict[str, Any]) -> list[dict[str, Any]]:
